...
test('Location creation workflow', async ({ page }) => {
    ...
    await logStep('When the user selects location type', () =>
        locationPage.whenTheUSerSelectsThisLocationType('Hospitality')
    );
    await logStep('When the user enters number of users', () => locationPage.whenTheUserEntersNumberOfUsers('10'));
    await logStep('When the user enters address', () => locationPage.whenTheUserEntersAddress('Pakhusgården 28'));
